...
class Electricbill extends CI_Controller
{
    ...
    public function index()
    {
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['role'])) {
                $bill_data = $this->Billpay_model->getBillPayData();
                $total_amount = $this->Billpay_model->totalAmount();
            }
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($users)) {
                        ...
                        $bill_data = $this->Billpay_model->getElectricDataByMobile($users);

                        $total_amount = $this->Billpay_model->totalBillAmountByMobile($users);
                    }
                } else {

                    $bill_data = $this->Billpay_model->getElectricData($pay_by);
            ...
            $services = $this->Billpay_model->getService();
            ...
            $this->load->view('electricbill', $data);
    ...
    public function Payment()
    {
        ...
        if (empty($cust_no) || empty($service_code) || empty($amount)) {
            $this->session->set_flashdata('error', 'Please fill in all the fields.');
            redirect('electricbill');
        }
    ...
    public function status()
    {
        ...
        $pending_data = $this->db->where_in('trans_status_code', $trans_status_codes)->get('tbl_electric_bill_pay')->result_array();
