...
class Fastagrecharge extends CI_Controller
{
    ...
    public function index()
    {
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['role'])) {
                $bill_data = $this->Fastag_model->getFastagPayData();
                $total_amount = $this->Fastag_model->totalAmount();
            }
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($merge) && !empty($merge)) {
                        ...
                        $bill_data = $this->Fastag_model->getFastagDataByMobile($merge);

                        $total_amount = $this->Fastag_model->totalBillAmountByMobile($merge);
                    }
                } else {
            ...
            $services = $this->Fastag_model->getService();
            ...
            $this->load->view('fastagrecharge', $data);
    ...
    //     $custNo = $this->input->post('cust_id');
    ...
    //     // $apiUrl = 'https://www.payoneapi.com/RechargeAPI/RechargeAPI.aspx?MobileNo=' . $mobileNo . '&APIKey=' . $apiKey . '&REQTYPE=BILLINFO&SERCODE=' . $serviceCode . '&CUSTNO=' . $custNo . '&REFMOBILENO=' . $mobileNo . '&AMT=10&STV=0&FIELD1=0&FIELD2=0&FIELD3=0&FIELD4=0&FIELD5=0&PCODE=' . $pincode . '&LAT=' . $latitude . '&LONG=' . $longitude . '&RESPTYPE=JSON';

    //     // ECHO $apiUrl; die;

    //     $apiUrl = 'https://www.payoneapi.com/RechargeAPI/RechargeAPI.aspx?MobileNo=8777846136&APIKey=boPpbGBPpxLiim5Qi9dnqy2onioy3ZaOckZ&REQTYPE=BILLINFO&SERCODE=HDFC&CUSTNO=WB34AJ9188&REFMOBILENO=8777846136&AMT=100&STV=0&FIELD1=0&FIELD2=0&FIELD3=0&FIELD4=0&FIELD5=0&PCODE=700086&LAT=22.487547&LONG=88.352768&RESPTYPE=JSON';
    ...
    public function Recharge()
    {
        ...
        if (empty($cust_id) || empty($service_code) || empty($amount)) {
            $this->session->set_flashdata('error', 'Please fill in all the fields.');
            redirect('fastagrecharge');
        }
    ...
    public function status()
    {
        ...
        $pending_data = $this->db->where_in('trans_status_code', $trans_status_codes)->get('tbl_fastag_recharge')->result_array();
        ...
        if (!empty($pending_data)) {
            foreach ($pending_data as $data) {
                ...
                $recharge_by = $data['recharge_by'];

                $user_data = $this->db->get_where('users', ['mobile' => $recharge_by])->row_array();
                ...
                if ($status == 1) {
                    ...
                    if (!empty($com_data)) {
                        $this->db->where(['mobile' => $recharge_by]);
                        $this->db->update('users', $com_data);
                    }

                    $input = [
                        'from_mobile' => $recharge_by,
                        'mobile' => $data['customer_id'],
                ...
                //         'amount' => $data['amount'],
                //         'balance' => $new_wallet_balance,
                //         'remarks' => 'FASTag Recharge Amount Refund'
                //     ];
