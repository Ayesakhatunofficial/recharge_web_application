...
class Insurance extends CI_Controller
{
    ...
    public function index()
    {
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['role'])) {
                $bill_data = $this->Insurance_model->getInsurancePayData();
                $total_amount = $this->Insurance_model->totalAmount();
            }
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($merge) && !empty($merge)) {
                        ...
                        $bill_data = $this->Insurance_model->getInsuranceDataByMobile($merge);

                        $total_amount = $this->Insurance_model->totalBillAmountByMobile($merge);
                    }
                } else {
                    $bill_data = $this->Insurance_model->getInsuranceData($pay_by);

                    $total_amount = $this->Insurance_model->totalBillAmount($pay_by);
                }
            }
            // print_r($bill_data); die;

            $services = $this->Insurance_model->getService();
    ...
    public function getCustomerInfo()
    {
        ...
        $custNo = $this->input->post('insurance_no');
        $email = $this->input->post('email');
        $dob = $this->input->post('dob');

        $data = $this->Insurance_model->getData();
        $pincode = $data->pincode;
        $latitude = $data->latitude;
        $longitude = $data->longitude;
        $dateofbirth = date('d/m/Y', strtotime($dob));

        $apiUrl = 'https://www.payoneapi.com/RechargeAPI/RechargeAPI.aspx?MobileNo=' . $mobileNo . '&APIKey=' . $apiKey . '&REQTYPE=BILLINFO&SERCODE=' . $serviceCode . '&CUSTNO=' . $custNo . '&REFMOBILENO=' . $mobileNo . '&AMT=0&STV=0&FIELD1=' . $email . '&FIELD2=' . $dateofbirth . '&FIELD3=0&FIELD4=0&FIELD5=0&PCODE=' . $pincode . '&LAT=' . $latitude . '&LONG=' . $longitude . '&RESPTYPE=JSON';

        // ECHO $apiUrl; die;

        // $apiUrl = 'https://www.payoneapi.com/RechargeAPI/RechargeAPI.aspx?MobileNo=8777846136&APIKey=boPpbGBPpxLiim5Qi9dnqy2onioy3ZaOckZ&REQTYPE=BILLINFO&SERCODE=AFPLM&CUSTNO=13969296&REFMOBILENO=8972084133&AMT=0&STV=0&FIELD1=0&FIELD2=0&FIELD3=0&FIELD4=0&FIELD5=0&PCODE=700086&LAT=22.487547&LONG=88.352768&RESPTYPE=JSON';
    ...
    public function Payment()
    {
        ...
        if (empty($insurance_no) || empty($service_code) || empty($amount) || empty($dob) || empty($email)) {
        ...
        $user_1 = $this->Insurance_model->getUser($created_by_id);
        $user_1_createdById = $user_1->created_by_id;

        $user_1_type_id = $user_1->create_by;

        $user_2 = $this->Insurance_model->getUser($user_1_createdById);
        ...
        if ($wallet_balance >= $amount && $api_balance >= $amount) {
            ...
            $payment_url = $this->Insurance_model->getApi();
            $api_url = $payment_url->url;


            $apiUrl = $api_url . 'MobileNo=' . $mobileNo . '&APIKey=' . $apiKey . '&REQTYPE=BILLPAY&REFNO=' . $ref_no . '&SERCODE=' . $service_code . '&CUSTNO=' . $insurance_no . '&REFMOBILENO=' . $mobileNo . '&AMT=' . $amount . '&STV=0&FIELD1=' . $email . '&FIELD2=' . $dob . '&FIELD3=0&FIELD4=0&FIELD5=0&PCODE=' . $pincode . '&LAT=' . $latitude . '&LONG=' . $longitude . '&RESPTYPE=JSON';
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if (isset($created_by_id) && $create_by == 'admin') {
                    $commission_a  = $this->Insurance_model->getCommission($service_code, $id, $slug, $service, $created_by_id);
                }
                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                    $commission_b  =  $this->Insurance_model->getCommission($service_code, $id, $slug, $service, $user_1_createdById);
                }
                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                    $commission_c = $this->Insurance_model->getCommission($service_code, $id, $slug, $service, $user_2_createdById);
                }

                $commission_d = $this->Insurance_model->getCommission($service_code, $id, $slug, $service, $role_slug_id);
                ...
                if ($commission->created_by != 'super_admin') {
                    ...
                    $admin_commission  =  $this->Insurance_model->getCommission($service_code, $commission->created_by, $admin_type, $service, $role_slug_id);
            ...
            $result = $this->db->insert('tbl_insurance_pay', $data);
            ...
            if ($status == 0 || $status == 4 || $status == 6) {
                ...
                $this->db->update('tbl_insurance_pay', $data_1);
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($status == 1) {
                    ...
                    if (isset($created_by_id) && $created_by_id != 0) {
                        ...
                        if (isset($created_by_id) && $create_by == 'admin') {
                            $commission1_a  = $this->Insurance_model->getCommission($service_code, $created_by_id, $create_by, $service, $created_by_id);
                        }
                        if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                            $commission1_b  =  $this->Insurance_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_1_createdById);
                        }
                        if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                            $commission1_c = $this->Insurance_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_2_createdById);
                        }

                        $commission1_d = $this->Insurance_model->getCommission($service_code, $created_by_id, $create_by, $service, $role_slug_id);
                        ...
                        if ($user_1_createdById != 0) {
                            ...
                            if (isset($created_by_id) && $create_by == 'admin') {
                                $commission2_a = $this->Insurance_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $created_by_id);
                            }
                            if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                $commission2_b = $this->Insurance_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_1_createdById);
                            }
                            if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                $commission2_c = $this->Insurance_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_2_createdById);
                            }

                            $commission2_d = $this->Insurance_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $role_slug_id);
                            ...
                            if ($user_2_createdById != 0) {
                                ...
                                if (isset($created_by_id) && $create_by == 'admin') {
                                    $commission3_a = $this->Insurance_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $created_by_id);
                                }
                                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                    $commission3_b = $this->Insurance_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_1_createdById);
                                }
                                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                    $commission3_c = $this->Insurance_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_2_createdById);
                                }

                                $commission3_d = $this->Insurance_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $role_slug_id);
    ...
    public function status()
    {
        ...
        $pending_data = $this->db->where_in('trans_status_code', $trans_status_codes)->get('tbl_insurance_pay')->result_array();
            ...
            foreach ($pending_data as $data) {
                ...
                if ($status == 1) {
                    ...
                    $history = [
                        'date' => date('Y-m-d'),
                        'time' => date('H:i:s'),
                        'customer_id' => $data['insurance_number'],
