...
class Creditcard extends CI_Controller
{
    ...
    public function index()
    {
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['role'])) {
                $bill_data = $this->Creditcard_model->getCreditcardPayData();
                $total_amount = $this->Creditcard_model->totalAmount();
            }
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($merge) && !empty($merge)) {
                        ...
                        $bill_data = $this->Creditcard_model->getCreditcardDataByMobile($merge);

                        $total_amount = $this->Creditcard_model->totalBillAmountByMobile($merge);
                    }
                } else {
                    $bill_data = $this->Creditcard_model->getCreditcardData($pay_by);
                    $total_amount = $this->Creditcard_model->totalBillAmount($pay_by);
                }
            }
            // print_r($bill_data); die;

            $services = $this->Creditcard_model->getService();
    ...
    public function getCustomerInfo()
    {
        ...
        $custNo = $this->input->post('credit_no');

        $data = $this->Creditcard_model->getData();
    ...
    public function Payment()
    {
        ...
        if (empty($credit_no) || empty($service_code) || empty($amount)) {
        ...
        if ($wallet_balance >= $amount && $api_balance >= $amount) {
            ...
            $payment_url = $this->Creditcard_model->getApi();
            $api_url = $payment_url->url;


            $apiUrl = $api_url . 'MobileNo=' . $mobileNo . '&APIKey=' . $apiKey . '&REQTYPE=BILLPAY&REFNO=' . $ref_no . '&SERCODE=' . $service_code . '&CUSTNO=' . $credit_no . '&REFMOBILENO=' . $mobileNo . '&AMT=' . $amount . '&STV=0&FIELD1=8777846136&PCODE=' . $pincode . '&LAT=' . $latitude . '&LONG=' . $longitude . '&RESPTYPE=JSON';
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if (isset($created_by_id) && $create_by == 'admin') {
                    $commission_a = $this->Creditcard_model->getCommission($service_code, $id, $slug, $service, $created_by_id);
                }
                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                    $commission_b = $this->Creditcard_model->getCommission($service_code, $id, $slug, $service, $user_1_createdById);
                }
                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                    $commission_c = $this->Creditcard_model->getCommission($service_code, $id, $slug, $service, $user_2_createdById);
                }

                $commission_d = $this->Creditcard_model->getCommission($service_code, $id, $slug, $service, $role_slug_id);
                ...
                if ($commission->created_by != 'super_admin') {
                    ...
                    $admin_commission = $this->Creditcard_model->getCommission($service_code, $commission->created_by, $admin_type, $service, $role_slug_id);
            ...
            $result = $this->db->insert('tbl_creditcard', $data);
            ...
            if ($status == 0 || $status == 4 || $status == 6) {
                ...
                $this->db->update('tbl_creditcard', $data_1);
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($status == 1) {
                    ...
                    $input = [
                        ...
                        'remarks' => 'Credit Card Pay Profit'
                    ];
                    if (!empty($input)) {
                    ...
                    if (isset($create_by) && $created_by_id != 0) {
                        ...
                        if (isset($created_by_id) && $create_by == 'admin') {
                            $commission1_a = $this->Creditcard_model->getCommission($service_code, $created_by_id, $create_by, $service, $created_by_id);
                        }
                        if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                            $commission1_b = $this->Creditcard_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_1_createdById);
                        }
                        if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                            $commission1_c = $this->Creditcard_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_2_createdById);
                        }

                        $commission1_d = $this->Creditcard_model->getCommission($service_code, $created_by_id, $create_by, $service, $role_slug_id);
                        ...
                        if ($user_1_createdById != 0) {
                            ...
                            if (isset($created_by_id) && $create_by == 'admin') {
                                $commission2_a = $this->Creditcard_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $created_by_id);
                            }
                            if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                $commission2_b = $this->Creditcard_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_1_createdById);
                            }
                            if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                $commission2_c = $this->Creditcard_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_2_createdById);
                            }

                            $commission2_d = $this->Creditcard_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $role_slug_id);
                            ...
                            if ($user_2_createdById != 0) {
                                ...
                                if (isset($created_by_id) && $create_by == 'admin') {
                                    $commission3_a = $this->Creditcard_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $created_by_id);
                                }
                                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                    $commission3_b = $this->Creditcard_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_1_createdById);
                                }
                                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                    $commission3_c = $this->Creditcard_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_2_createdById);
                                }

                                $commission3_d = $this->Creditcard_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $role_slug_id);
    ...
    public function status()
    {
        ...
        $pending_data = $this->db->where_in('trans_status_code', $trans_status_codes)->get('tbl_creditcard')->result_array();
        ...
        if (!empty($pending_data)) {
            foreach ($pending_data as $data) {
                ...
                //         'amount' => $data['amount'],
                //         'balance' => $new_wallet_balance,
                //         'remarks' => 'Credit Card Pay Amount Refund'
                //     ];
