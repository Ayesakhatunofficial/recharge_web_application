...
class Dthrecharge extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($merge) && !empty($merge)) {
                        ...
                        $datas = $this->Recharge_model->getDthDataByMobile($merge);

                        $total_amount = $this->Recharge_model->totalDthRechargeAmount($merge);
                    }
                } else {
                    $datas = $this->Recharge_model->getDthhisData($recharge_by);

                    $total_amount = $this->Recharge_model->totaldthrechAmount($recharge_by);
                }
            }
            if (isset($_SESSION['role'])) {

                $datas = $this->Recharge_model->getDTHData();

                $total_amount = $this->Recharge_model->totalDTHAmount();
            }

            // print_r($datas);die;


            $operators = $this->Recharge_model->getDthOperator();
            $view_data = [
                'datas' => $datas,
            ...
            $this->load->view('dthrecharge', $view_data);
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://api.rechargeexchange.com/API.asmx/OperatorList');
