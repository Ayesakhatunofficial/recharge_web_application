...
class Panfinderpay extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_SESSION['user_type'])) {
            ...
            if ($_SESSION['order_id'] != NULL) {
                ...
                $online_panfinder_pay = $this->db->get_where('online_payment_history', ['order_id' => $_SESSION['order_id'], 'status' => 1])->row_array();

                $deduct_wallet_balance = $user_type['wallet_money'] - $online_panfinder_pay['Fees'];
                ...
                if ($deduct_wallet_balance > 0) {
                    ...
                    if ($error['code'] == 0) {
                        ...
                        if ($deduct_wallet_balance != $user_type['wallet_money']) {
                            ...
                            $this->db->update('panfinder', ['is_pay_print' => 1]);
                        }
                        ...
                        $this->db->insert('wallet_balance_history', [
                            ...
                            'time' => date('H:m:s'),
                            'cr_dr' => 'Debit',
                            'amount' => $online_panfinder_pay['Fees'],
                            'balance' => $deduct_wallet_balance,
                            'remarks' => $online_panfinder_pay['Remarks']
                        ]);
