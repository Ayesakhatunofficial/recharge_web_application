...
class Landline extends CI_Controller
{
    ...
    public function getCustInfo()
    {
        ...
        $this->form_validation->set_rules('landline_no', 'Landline Number', 'required');
    ...
    public function bill()
    {
        ...
        $landline_bill = $this->db->query(" SELECT 
                                            tbl_landline_operator.op_logo, 
                                            tbl_landline_operator.operator,
                                            users.name,
                                            user_type.user_type,
                                            tbl_landline_pay.*
                                        FROM 
                                            tbl_landline_pay
                                        JOIN 
                                            tbl_landline_operator
                                        ON 
                                            tbl_landline_operator.opcode = tbl_landline_pay.service_code
                                        JOIN 
                                            users
                                        ON 
                                            tbl_landline_pay.pay_by =  users.mobile 
                                        JOIN 
                                            user_type
                                        ON 
                                            users.account_type = user_type.id           
                                        WHERE 
                                            tbl_landline_pay.id = $id")->row_array();
        ...
        $amount = $landline_bill['amount'];

        $words = AmountInWords($amount);

        $words = str_replace(array("\r", "\n"), '', $words);

        if (!is_null($landline_bill)) {
