...
class Broadband extends CI_Controller
{
    ...
    public function getCustInfo()
    {
        ...
        $this->form_validation->set_rules('broadband_number', 'Broadband Number', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $custNo = $this->input->post('broadband_number');
    ...
    public function bill()
    {
        ...
        $broadband_bill = $this->db->query(" SELECT 
                                        ...
                                        FROM 
                                            tbl_broadband
                                        JOIN 
                                            tbl_broadband_operator
                                        ON 
                                            tbl_broadband_operator.opcode = tbl_broadband.service_code
                                        JOIN 
                                            users
                                        ON 
                                            tbl_broadband.pay_by =  users.mobile 
                                        JOIN 
                                            user_type
                                        ON 
                                            users.account_type = user_type.id           
                                        WHERE 
                                            tbl_broadband.id = $id")->row_array();
        ...
        $amount = $broadband_bill['amount'];
        $words = AmountInWords($amount);

        $words = str_replace(array("\r", "\n"), '', $words);

        if (!is_null($broadband_bill)) {
            $respond = [
                'status' => true,
                'message' => 'Success',
                'data' => $broadband_bill,
