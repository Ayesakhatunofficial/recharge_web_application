...
class Cable extends CI_Controller
{
    ...
    public function index()
    {
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['role'])) {
                $bill_data = $this->Cable_model->getCablePayData();
                $total_amount = $this->Cable_model->totalAmount();
            }
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($merge) && !empty($merge)) {
                        ...
                        $bill_data = $this->Cable_model->getCableDataByMobile($merge);

                        $total_amount = $this->Cable_model->totalBillAmountByMobile($merge);
                    }
                } else {
                    $bill_data = $this->Cable_model->getCableData($pay_by);

                    $total_amount = $this->Cable_model->totalBillAmount($pay_by);
                }
            }

            $services = $this->Cable_model->getService();
    ...
    public function getCustomerInfo()
    {
        ...
        $custNo = $this->input->post('cable_no');

        $data = $this->Cable_model->getData();
    ...
    {
        $cable_no = $this->input->post('cable_no');
        $service_code = $this->input->post('service_code');
        $amount = $this->input->post('amount');
        $cust_name = $this->input->post('cust_name');
        $due_date = $this->input->post('due_date');

        if (empty($cable_no) || empty($service_code) || empty($amount)) {
        ...
        $user_1 = $this->Cable_model->getUser($created_by_id);
        $user_1_createdById = $user_1->created_by_id;

        $user_1_type_id = $user_1->create_by;

        $user_2 = $this->Cable_model->getUser($user_1_createdById);
        ...
        if ($wallet_balance >= $amount && $api_balance >= $amount) {
            ...
            $payment_url = $this->Cable_model->getApi();
            $api_url = $payment_url->url;


            $apiUrl = $api_url . 'MobileNo=' . $mobileNo . '&APIKey=' . $apiKey . '&REQTYPE=BILLPAY&REFNO=' . $ref_no . '&SERCODE=' . $service_code . '&CUSTNO=' . $cable_no . '&REFMOBILENO=' . $mobileNo . '&AMT=' . $amount . '&STV=0&FIELD1=8777846136&PCODE=' . $pincode . '&LAT=' . $latitude . '&LONG=' . $longitude . '&RESPTYPE=JSON';
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if (isset($created_by_id) && $create_by == 'admin') {
                    $commission_a = $this->Cable_model->getCommission($service_code, $id, $slug, $service, $created_by_id);
                }
                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                    $commission_b = $this->Cable_model->getCommission($service_code, $id, $slug, $service, $user_1_createdById);
                }
                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                    $commission_c = $this->Cable_model->getCommission($service_code, $id, $slug, $service, $user_2_createdById);
                }

                $commission_d = $this->Cable_model->getCommission($service_code, $id, $slug, $service, $role_slug_id);
                ...
                if ($commission->created_by != 'super_admin') {
                    ...
                    $admin_commission = $this->Cable_model->getCommission($service_code, $commission->created_by, $admin_type, $service, $role_slug_id);
            ...
            $result = $this->db->insert('tbl_cable', $data);
            ...
            if ($status == 0 || $status == 4 || $status == 6) {
                ...
                $this->db->update('tbl_cable', $data_1);
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($status == 1) {
                    ...
                    if (isset($create_by) && $created_by_id != 0) {
                        ...
                        if (isset($created_by_id) && $create_by == 'admin') {
                            $commission1_a = $this->Cable_model->getCommission($service_code, $created_by_id, $create_by, $service, $created_by_id);
                        }
                        if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                            $commission1_b = $this->Cable_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_1_createdById);
                        }
                        if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                            $commission1_c = $this->Cable_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_2_createdById);
                        }

                        $commission1_d = $this->Cable_model->getCommission($service_code, $created_by_id, $create_by, $service, $role_slug_id);
                        ...
                        if ($user_1_createdById != 0) {
                            ...
                            if (isset($created_by_id) && $create_by == 'admin') {
                                $commission2_a = $this->Cable_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $created_by_id);
                            }
                            if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                $commission2_b = $this->Cable_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_1_createdById);
                            }
                            if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                $commission2_c = $this->Cable_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_2_createdById);
                            }

                            $commission2_d = $this->Cable_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $role_slug_id);
                            ...
                            if ($user_2_createdById != 0) {
                                ...
                                if (isset($created_by_id) && $create_by == 'admin') {
                                    $commission3_a = $this->Cable_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $created_by_id);
                                }
                                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                    $commission3_b = $this->Cable_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_1_createdById);
                                }
                                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                    $commission3_c = $this->Cable_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_2_createdById);
                                }

                                $commission3_d = $this->Cable_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $role_slug_id);
    ...
    public function status()
    {
        ...
        $pending_data = $this->db->where_in('trans_status_code', $trans_status_codes)->get('tbl_cable')->result_array();
            ...
            foreach ($pending_data as $data) {
                ...
                if ($status == 2 || $status == 3 || $status == 5) {
                    ...
                    $wallet_history = [
                        ...
                        'amount' => $data['amount'],
                        'balance' => $new_wallet_balance,
                        'remarks' => 'Cable Recharge Amount Refund'
                    ];
