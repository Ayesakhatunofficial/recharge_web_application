...
class Municiple extends CI_Controller
{
    ...
    public function index()
    {
        if ($_SESSION['role'] || $_SESSION['slug']) {
            ...
            if (isset($_SESSION['role'])) {
                $bill_data = $this->Municiple_model->getMunicipleData();
                $total_amount = $this->Municiple_model->totalAmount();
            }
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($_SESSION['slug'] == 'admin') {
                    ...
                    if (!is_null($merge) && !empty($merge)) {
                        ...
                        $bill_data = $this->Municiple_model->getMunicipleDataByMobile($merge);

                        $total_amount = $this->Municiple_model->totalBillAmountByMobile($merge);
                    }
                } else {
                    $bill_data = $this->Municiple_model->getServiceData($pay_by);
                    $total_amount = $this->Municiple_model->totalBillAmount($pay_by);
                }
            }
            // print_r($bill_data); die;

            $services = $this->Municiple_model->getService();
    ...
    public function getCustomerInfo()
    {
        ...
        $custNo = $this->input->post('service_no');

        $data = $this->Municiple_model->getData();
    ...
    public function Servicepay()
    {
        ...
        if (empty($service_no) || empty($service_code) || empty($amount)) {
        ...
        $user_1 = $this->Municiple_model->getUser($created_by_id);
        $user_1_createdById = $user_1->created_by_id;

        $user_1_type_id = $user_1->create_by;

        $user_2 = $this->Municiple_model->getUser($user_1_createdById);
        ...
        if ($wallet_balance >= $amount && $api_balance >= $amount) {
            ...
            $payment_url = $this->Municiple_model->getApi();
            $api_url = $payment_url->url;


            $apiUrl = $api_url . 'MobileNo=' . $mobileNo . '&APIKey=' . $apiKey . '&REQTYPE=BILLPAY&REFNO=' . $ref_no . '&SERCODE=' . $service_code . '&CUSTNO=' . $service_no . '&REFMOBILENO=' . $mobileNo . '&AMT=' . $amount . '&STV=0&FIELD1=8777846136&PCODE=' . $pincode . '&LAT=' . $latitude . '&LONG=' . $longitude . '&RESPTYPE=JSON';
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if (isset($created_by_id) && $create_by == 'admin') {
                    $commission_a = $this->Municiple_model->getCommission($service_code, $id, $slug, $service, $created_by_id);
                }
                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                    $commission_b = $this->Municiple_model->getCommission($service_code, $id, $slug, $service, $user_1_createdById);
                }
                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                    $commission_c = $this->Municiple_model->getCommission($service_code, $id, $slug, $service, $user_2_createdById);
                }

                $commission_d = $this->Municiple_model->getCommission($service_code, $id, $slug, $service, $role_slug_id);
                ...
                if ($commission->created_by != 'super_admin') {
                    ...
                    $admin_commission = $this->Municiple_model->getCommission($service_code, $commission->created_by, $admin_type, $service, $role_slug_id);
            ...
            $result = $this->db->insert('tbl_municiple_pay', $data);
            ...
            if ($status == 0 || $status == 4 || $status == 6) {
                ...
                $this->db->update('tbl_municiple_pay', $data_1);
            ...
            if (isset($_SESSION['slug'])) {
                ...
                if ($status == 1) {
                    ...
                    if (isset($create_by) && $created_by_id != 0) {
                        ...
                        if (isset($created_by_id) && $create_by == 'admin') {
                            $commission1_a = $this->Municiple_model->getCommission($service_code, $created_by_id, $create_by, $service, $created_by_id);
                        }
                        if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                            $commission1_b = $this->Municiple_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_1_createdById);
                        }
                        if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                            $commission1_c = $this->Municiple_model->getCommission($service_code, $created_by_id, $create_by, $service, $user_2_createdById);
                        }

                        $commission1_d = $this->Municiple_model->getCommission($service_code, $created_by_id, $create_by, $service, $role_slug_id);
                        ...
                        if ($user_1_createdById != 0) {
                            ...
                            if (isset($created_by_id) && $create_by == 'admin') {
                                $commission2_a = $this->Municiple_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $created_by_id);
                            }
                            if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                $commission2_b = $this->Municiple_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_1_createdById);
                            }
                            if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                $commission2_c = $this->Municiple_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $user_2_createdById);
                            }

                            $commission2_d = $this->Municiple_model->getCommission($service_code, $user_1_createdById, $c_by, $service, $role_slug_id);
                            ...
                            if ($user_2_createdById != 0) {
                                ...
                                if (isset($created_by_id) && $create_by == 'admin') {
                                    $commission3_a = $this->Municiple_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $created_by_id);
                                }
                                if (isset($user_1_createdById) && $user_1_type_id == 'admin') {
                                    $commission3_b = $this->Municiple_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_1_createdById);
                                }
                                if (isset($user_2_createdById) && $user_2_type_id == 'admin') {
                                    $commission3_c = $this->Municiple_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $user_2_createdById);
                                }

                                $commission3_d = $this->Municiple_model->getCommission($service_code, $user_2_createdById, $c_by_1, $service, $role_slug_id);
    ...
    public function status()
    {
        ...
        $pending_data = $this->db->where_in('trans_status_code', $trans_status_codes)->get('tbl_municiple_pay')->result_array();
            ...
            foreach ($pending_data as $data) {
                ...
                if ($status == 1) {
                    ...
                    $history = [
                        'date' => date('Y-m-d'),
                        'time' => date('H:i:s'),
                        'customer_id' => $data['service_number'],
                ...
                //         'amount' => $data['amount'],
                //         'balance' => $new_wallet_balance,
                //         'remarks' => 'Municiple Pay Amount Refund'
                //     ];
