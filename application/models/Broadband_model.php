<?php
class Broadband_model extends CI_Model
{
    public function getService()
    {
        $sql = "SELECT * FROM tbl_broadband_operator";
    ...
    public function getBroadbandPayData()
    {
        $sql = "SELECT * FROM tbl_broadband ORDER BY id DESC";
        return $this->db->query($sql)->result();
    }

    public function getBroadbandData($pay_by)
    {
        $sql = "SELECT 
                    tbl_broadband_operator.operator,
                    tbl_broadband_operator.op_logo,
                    tbl_broadband.* 
                FROM 
                    tbl_broadband
                JOIN tbl_broadband_operator ON tbl_broadband_operator.opcode = tbl_broadband.service_code 
                WHERE tbl_broadband.pay_by = $pay_by 
                ORDER BY tbl_broadband.id DESC";
        return $this->db->query($sql)->result();
    }
    ...
    public function totalAmount()
    {
     ...
     FROM
     tbl_broadband WHERE trans_status_code = 1";
        return $this->db->query($sql)->row();
    }
    ...
    public function getBroadbandPayReport($from_date, $to_date, $opcode)
    {
    ...
    public function getBroadbandReport($from_date, $to_date, $opcode, $pay_by)
    {
    ...
    public function getBroadbandReportByMobile($from_date, $to_date, $opcode, $users)
    {
    ...
    public function getBroadbandDataByMobile($mobileNumbers)
    {
        $this->db->select('*');
        $this->db->from('tbl_broadband');
