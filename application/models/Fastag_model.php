<?php
class Fastag_model extends CI_Model
{
    ...
    {
        $sql = "SELECT * FROM tbl_fastag_operator";
    ...
    public function getFastagPayData()
    {
        $sql = "SELECT * FROM tbl_fastag_recharge ORDER BY id DESC";
        return $this->db->query($sql)->result();
    }

    public function getFastagData($pay_by)
    {
        $sql = "SELECT
     ...
     FROM
        tbl_fastag_recharge
     JOIN tbl_fastag_operator ON tbl_fastag_recharge.service_code = tbl_fastag_operator.opcode
     WHERE
        tbl_fastag_recharge.recharge_by = $pay_by
     ORDER BY
        tbl_fastag_recharge.id
     DESC
        ";
        return $this->db->query($sql)->result();
    }
    ...
    public function totalAmount()
    {
        ...
        SUM(dp) AS total_api_profit
     FROM
     tbl_fastag_recharge WHERE trans_status_code = 1";
        return $this->db->query($sql)->row();
    }
    ...
    public function totalBillAmount($pay_by)
    {
     ...
     tbl_fastag_recharge
     WHERE
     recharge_by = $pay_by AND trans_status_code = 1";
        return $this->db->query($sql)->row();
    }

    public function getFastagPayReport($from_date, $to_date, $opcode)
    {
        $params = array($from_date, $to_date, $opcode, $opcode);
        // print_r($params);die;
        $sql = "SELECT *
                FROM tbl_fastag_recharge
    ...
    public function getFastagReport($from_date, $to_date, $opcode, $pay_by)
    {
    ...
    public function getFastagReportByMobile($from_date, $to_date, $opcode, $users)
    {
    ...
    public function getFastagDataByMobile($mobileNumbers)
    {
        $this->db->select('*');
        $this->db->from('tbl_fastag_recharge');
