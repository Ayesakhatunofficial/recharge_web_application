<?php
class Landline_model extends CI_Model
{
    public function getService()
    {
        $sql = "SELECT * FROM tbl_landline_operator";
    ...
    public function getLandlinePayData()
    {
        $sql = "SELECT * FROM tbl_landline_pay ORDER BY id DESC";
        return $this->db->query($sql)->result();
    }

    public function getLandlineData($pay_by)
    {
        $sql = "SELECT 
                ...
                FROM 
                    tbl_landline_pay 
                JOIN 
                    tbl_landline_operator ON tbl_landline_pay.service_code = tbl_landline_operator.opcode
                WHERE 
                    tbl_landline_pay.pay_by = $pay_by 
                ORDER BY 
                    tbl_landline_pay.id DESC";
        return $this->db->query($sql)->result();
    }
    ...
    public function totalAmount()
    {
     ...
     FROM
     tbl_landline_pay WHERE trans_status_code = 1";
        return $this->db->query($sql)->row();
    }
    ...
    public function getLandlinePayReport($from_date, $to_date, $opcode)
    {
        $params = array($from_date, $to_date, $opcode, $opcode);
        // print_r($params);die;
        $sql = "SELECT *
                FROM tbl_landline_pay
    ...
    public function getLandlineReport($from_date, $to_date, $opcode, $pay_by)
    {
    ...
    public function getLandlineReportByMobile($from_date, $to_date, $opcode, $users)
    {
    ...
    public function getLandlineDataByMobile($mobileNumbers)
    {
        $this->db->select('*');
        $this->db->from('tbl_landline_pay');
